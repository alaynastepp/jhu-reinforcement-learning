...
def plot_state_action_distribution_logscale(visit_count, agent_name, save_path=None):
    plt.figure(figsize=(12, 6))
    visit_counts_per_action = np.sum(visit_count, axis=0)
    
    plt.bar(range(len(visit_counts_per_action)), visit_counts_per_action, 
            tick_label=["Stay Still", "Move Up", "Move Down"], 
            color='teal', alpha=0.7)
    
    plt.yscale('log')  # Apply logarithmic scale
    plt.xlabel("Actions")
    plt.ylabel("Log Scale Visit Count")
    plt.title(f"{agent_name} - Logarithmic Distribution of Visit Count per Action")
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    
    if save_path:
        file_path = os.path.join(save_path, f"{agent_name.lower()}_state_action_distribution_log.png")
        plt.savefig(file_path)
    #plt.show()
    plt.close()
...
def plot_state_action_distribution_all_logscale(visit_counts, agent_names, save_path=None):
    num_agents = len(agent_names)
    actions = ["Stay Still", "Move Up", "Move Down"]
    num_actions = len(actions)
    ...
    plt.xlabel("Actions")
    plt.ylabel("Log Scale Visit Count")
    plt.title("State-Action Visit Counts per Agent (Log Scale)")
    plt.xticks(x + bar_width * (num_agents - 1) / 2, actions)  # Center action labels
    plt.legend(title="Agents")
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    ...
    # Save or display the plot
    if save_path:
        file_path = os.path.join(save_path, "all_state_action_visits_log.png")
        plt.savefig(file_path)
    #plt.show()
    plt.close()
